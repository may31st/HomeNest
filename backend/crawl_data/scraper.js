            ...
            const scraperDetail = async (link) => {
                return new Promise(async (resolve, reject) => {
                    try {
                        ...
                        const room_images = await pageDetails.$$eval('#carousel_Photos .carousel-inner img', (els) => {
                            return els.map(img => img.getAttribute('src') || img.getAttribute('data-src') || img.src).filter(src => !!src);
                        });
                            ...
                            return els.map(el => {
                                ...
                                for (const row of rows) {
                                    ...
                                    if (label.includes('Địa chỉ')) {
                                        addressText = row.querySelector('td:last-child')?.innerText || '';
                                        break;
                                    }
                                }
                                return {
                                    room_name: el.querySelector('h1')?.innerText || '', // Sử dụng optional chaining
                                    address: addressText.trim() || el.querySelector('address')?.innerText || '', // Thử lấy từ bảng hoặc fallback sang tag address
